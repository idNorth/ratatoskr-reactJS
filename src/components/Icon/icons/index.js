export { ReactComponent as CookingPotRegular } from "./cookingPot/regular.svg";
export { ReactComponent as CookingPotSolid } from "./cookingPot/solid.svg";
export { ReactComponent as GearRegular } from "./gear/regular.svg";
export { ReactComponent as GearSolid } from "./gear/solid.svg";
export { ReactComponent as GridRegular } from "./grid/regular.svg";
export { ReactComponent as ListRegular } from "./list/regular.svg";

export { ReactComponent as AppLogo } from "./appLogo.svg";
